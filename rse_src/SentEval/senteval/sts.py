...
from senteval.similarity_function import sim_max_func, sim_mean_func, sim_rel_func, sim_weight_func
        ...
        for dataset in self.datasets:
            ...
            for ii in range(0, len(gs_scores), params.batch_size):
                ...
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    if 'sim_func' in params:
                        enc1, rel_emb = batcher(params, batch1, rel_emb_out=True)
                    else:
                    ...
                    for kk in range(enc2.shape[0]):
                        ...
                        if 'sim_func' in params:
                            ...
                            if params['sim_func'][0] == 'max':
                                sys_score = sim_max_func(enc1[kk], enc2[kk], rel_emb, self.similarity)
                            else:
                                sys_score = sim_rel_func(enc1[kk], enc2[kk], rel_emb, self.similarity, params['sim_func'])
...
class STS13Eval(STSEval):
    ...
    def __init__(self, taskpath, seed=1111):
        logging.info('***** Transfer task : STS13 (-SMT) *****')
        self.seed = seed
...
class STSBenchmarkEval_unsup(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.info('***** Transfer task : STSBenchmark Unsupervised *****')
        self.seed = seed
        self.samples = []
        train = self.loadFile(os.path.join(task_path, 'sts-train.csv'))
        dev = self.loadFile(os.path.join(task_path, 'sts-dev.csv'))
...
class SICKRelatednessEval_unsup(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.info('***** Transfer task : SICKRelatedness Unsupervised *****')
        self.seed = seed
        self.samples = []
...
class STR(STSEval):
    def __init__(self, task_path, seed=1111):
        logging.info('***** Transfer task : STR Unsupervised *****')
        self.seed = seed
        self.samples = []
        test = self.loadFile(os.path.join(task_path, 'sem_text_rel_ranked.csv'))
        self.datasets = ['test']
        self.data = {'test': test}
