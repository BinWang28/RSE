...
class PROBINGEval(object):
    def __init__(self, task, task_path, seed=1111):
        self.seed = seed
        self.task = task
        logging.info('***** (Probing) Transfer task : %s classification *****', self.task.upper())
        self.task_data = {'train': {'X': [], 'y': []},
                          'dev': {'X': [], 'y': []},
                          'test': {'X': [], 'y': []}}
        self.loadFile(task_path)
        logging.debug('Loaded %s train - %s dev - %s test for %s' %
    ...
    def run(self, params, batcher):
        ...
        logging.debug('Computing embeddings for train/dev/test')
