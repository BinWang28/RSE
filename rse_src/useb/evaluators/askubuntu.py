...
class AskubuntuEvaluator(BaseEvaluator, AskUbuntuData):
    ...
    def __init__(self, semb_fn, datasets_dir=None, text_components='title', bsz=32, show=True):
    ...
    def _run(self, eval_type):
        ...
        for eval_example in tqdm.tqdm(eval_set, disable=not self.show):
            ...
            embs = self._text2se(sents, normalize=False)  # (bsz, hdim) = (1+20, hdim)
            ...
            for rel_emb in rel_embs:
                ...
                score = cos_sim(qembs_s, cembs)
                scores_rel.append(score.numpy())
            scores_rel = np.array(scores_rel)

            scores = rel_weights.dot(scores_rel).tolist()
